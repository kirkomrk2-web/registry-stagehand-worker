...
export const useChatLogic = ({ isChatOpen, initialFlow, setInitialFlow }) => {
    ...
    const addMessage = useCallback((sender, text, messageOptions = {}) => {
        ...
        if (sender === 'bot') {
            ...
            // ⭐ FIX: Variable delay for more realistic feel
    ...
    const handleConversationFlow = useCallback(async (userInput, currentSession) => {
        ...
        // ⭐ FIX: Realistic typing with variable delays
        const simulateTyping = (callback, delay = null) => {
            if (delay === null) {
                // Calculate delay based on message length for realism
        ...
        // ⭐ FIX 1: Add "Вход" and "Контакти" button handlers
        ...
        switch (step) {
            ...
            case 'lastName': {
                ...
                if (isSpam(userInput) || !validation.isValid) {
                    // ⭐ FIX: Show input field again after error
            ...
            case 'email': {
                ...
                // Start Supabase Direct Insertion Flow
                setIsSavingProfile(true);
                
                try {
                    // Format date for DB (assuming DD.MM.YYYY -> YYYY-MM-DD for consistency)
                    ...
                    // ⭐ FIX 2: Better message about processing time
                    simulateTyping(() => {
                        addBotMessage({
                    ...
                    // Add a small delay before starting registry check for UX
                    await new Promise(resolve => setTimeout(resolve, 1500));
                    
                    // 3. Trigger Registry Check (Edge Function)
                    await checkRegistry({ full_name: fullName, email: finalUserData.email });
                    
                    // 4. Trigger users_pending_worker
                    ...
                    // 5. ⭐ FIX: Go directly to dataSent, skip FinalizationComplete
                    simulateTyping(() => { 
    ...
    const handleGreeting = useCallback((agentName) => {
        ...
        setTimeout(() => {
            ...
            setIsTyping(false);
            // ⭐ FIX: Longer pause after greeting for realism
            setTimeout(() => {
                setIsTyping(true);
                setTimeout(() => {
                    ...
                    setIsTyping(false);
                }, 1500); // Increased from 800 to 1500
            }, 2000); // Increased from 1200 to 2000
        }, 1800); // Increased from 1500 to 1800
