...
export const useChatLogic = ({ isChatOpen, initialFlow, setInitialFlow }) => {
    ...
    const handleConversationFlow = useCallback(async (userInput, currentSession) => {
        ...
        // ⭐ FIX: "Контакти" button handler with dynamic agent info (NO addMessage)
        if (userInput === "Контакти") {
            const agentEmail = `${agent.nameEn}@wallester-bg.com`;
            const agentTelegram = agent.telegram;
            const telegramMessage = encodeURIComponent("Здравей, имам нужда от твоята помощ");
            const telegramLink = `https://t.me/${agentTelegram}?text=${telegramMessage}`;
            ...
            simulateTyping(() => {
                addBotMessage({
                    ...
                    options: [
                        { text: "Пиши в Telegram", icon: "MessageCircle", type: "link", url: telegramLink }
                    ]
                });
            }, 1000);
            return;
